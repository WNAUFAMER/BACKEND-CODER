...
export class CartManager{
  #ruta = './src/carrito.json'
  constructor(){
    this.products = []
    this.path = this.#ruta
  }
  
  getCartProducts = async (carID) => {
    try {
      if (fs.existsSync(this.path)) {
        const data = await fs.promises.readFile(this.path, 'utf-8')
        const productDb = JSON.parse(data);
        const carrito = productDb[parseInt(carID) - 1]
        return carrito;
      }
  ...
  addToCart = async (carID, proID) => {
    try {
      const data = await fs.promises.readFile(this.path, 'utf-8')
      const cartDb = JSON.parse(data)
      const carrito = cartDb[parseInt(carID) - 1]
      const idx = carrito.products.findIndex(product => product.id === parseInt(proID))
      if (idx !== -1) {
        ...
        const product = {}
        product.id = parseInt(proID)
        product.quantity = 1
        carrito.products = [...carrito.products, product]
      }

      cartDb[parseInt(carID) - 1] = carrito
